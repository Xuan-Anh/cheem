...
def water_irrigate():
	...
	df_details['Nước cỏ'] = df_details['Nhiệt độ'].apply(lambda x: 
			climates[(climates['Nhiệt độ trung bình hằng ngày'] =='Thấp (dưới 15 ° C)') & (climates['Vùng khí hậu'] == 'Ẩm ướt')]['Lượng nước'] 
		if x < 15 else 
			climates[(climates['Nhiệt độ trung bình hằng ngày'] =='Cao (hơn 25 ° C)') & (climates['Vùng khí hậu'] == 'Ẩm ướt')]['Lượng nước']
		if x > 25 else
			climates[(climates['Nhiệt độ trung bình hằng ngày'] =='Trung bình (15-25 ° C)') & (climates['Vùng khí hậu'] == 'Ẩm ướt')]['Lượng nước']
		)
	
	water_coef_tree_np = water_coef[water_coef['Mùa vụ'].str.contains(tree)].to_numpy()
	...
	absorbing_plant = rain_fall.apply(lambda x: (x*0.8 -25) if x>75 else 0 if(x*0.6 < 10) else (x*0.6 -10)).astype(float)
	...
	water_need_irrigate = pd.DataFrame(zip(first_stage,grow_stage,mid_stage,final_stage),columns=['Giai đoạn đầu','Giai đoạn phát triển cây trồng','Giai đoạn giữa mùa','Giai đoạn cuối mùa'])
